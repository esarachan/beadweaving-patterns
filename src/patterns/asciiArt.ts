...
const rowToAsciiArt = (row: Bead[], firstColumnIsElevated: boolean, colorful: boolean) => {
  ...
  const evenBeadsAscii = evenBeads.map(bead => beadToString(bead, colorful)).join(' ');
  const oddBeadsAscii = ' ' + oddBeads.map(bead => beadToString(bead, colorful)).join(' ');
...
export const patternToAsciiArt = (pattern: PeyotePattern, colorful: boolean) => {
  const rowAsciis = pattern.rows.map(row => rowToAsciiArt(row, pattern.firstColumnIsElevated, colorful));
